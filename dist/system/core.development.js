System.register([ "redux-thunk" ], (function(exports) {
    "use strict";
    var thunkMiddleware;
    return {
        setters: [ function(module) {
            thunkMiddleware = module.default;
        } ],
        execute: function() {
            exports({
                compose: compose,
            ...
            function createEnhancer({notifier: notifier, partMap: partMap}) {
            ...
            function createReducer({otherReducer: otherReducer, partMap: partMap, parts: parts}) {
                ...
                return function reducer(state, action) {
                    ...
                    const nextOtherState = additionalReducer(state, action);
                    return is(state, nextOtherState) ? state : {
                        ...state,
                        ...nextOtherState
                    };
                };
            ...
            function getInitialState(parts) {
                ...
                for (let index = 0; index < parts.length; ++index) {
                    ...
                    initialState[part.n] = part.r(void 0, {});
                }
                return initialState;
            }
            ...
            function createStore(options) {
                const {devTools: devTools = true, enhancers: enhancers, middleware: middleware = [ thunkMiddleware ], notifier: notifier = defaultNotifier, otherReducer: otherReducer, parts: parts, preloadedState: preloadedState} = options || {};
                const partMap = getStatefulPartMap(parts);
                const partsEnhancer = createEnhancer({
                    notifier: notifier,
                    partMap: partMap
                });
            ...
            function createBoundSelector(parts, get, isEqual) {
                ...
                return function select(getState, getVersion) {
                    ...
                    if (hasChanged) {
                        ...
                        if (hasPromise) {
                            const nextResult = Promise.all(nextValues).then((resolvedValues => get(...resolvedValues)));
            ...
            function createComposedReducer(part2) {
                ...
                return function reduce(state = initialState, action) {
                    ...
                    return is(prevState, nextState) ? state : {
                        ...state,
                        [name]: nextState
                    };
                };
            ...
            function createComposedPart(config) {
                const {name: name, parts: parts} = config;
            ...
            function createPrimitivePart(config) {
                const {initialState: initialState, name: name} = config;
                const part2 = function actionCreator(nextValue) {
                    return {
                        $$part: part2.id,
                        type: part2.t,
            ...
            function createBoundSelectPart(config) {
                const {get: get, isEqual: isEqual = is, parts: parts} = config;
            ...
            function createUnboundSelectPart(config) {
                const {get: get, isEqual: isEqual = is} = config;
            ...
            function createBoundProxyPart(config) {
                const {get: get, isEqual: isEqual = is, parts: parts, set: set} = config;
                const select = createBoundSelector(parts, get, isEqual);
            ...
            function createUnboundProxyPart(config) {
                const {get: get, isEqual: isEqual = is, set: set} = config;
                const select = createUnboundSelector(get, isEqual);
                const update = createUpdate(set);
                const part2 = {};
                part2.id = getId();
            ...
            function createPartUpdater(part2) {
                return function partUpdater(baseType, getValue = identity) {
                    ...
                    const updatePart = createUpdatePart({
                        set: set
                    });
            ...
            function part(first, second, third) {
                ...
                throw new Error(`The parameters passed are invalid for creating a Part; received [${Array.from(arguments, (parameter => typeof parameter))}]`);
            }
        }
    };
}));
