...
function Legacy() {
  ...
  const [legacy, setLegacy] = useState<State['legacy']>(
    () => store.getState().legacy
  );

  useAfterTimeout(() => store.dispatch({ type: 'LEGACY' }), 2500);

  useEffect(() => {
    const listener = () => {
      setLegacy(store.getState().legacy);
    };

    return store.subscribe(listener);
  }, [store]);

  return <div>Legacy: {legacy}</div>;
}
...
function ConditionalTest() {
  const [todos, setTodos] = useSlice(todosSlice);
  const [fullName, setFullName] = useSlice(fullNameSelect);
  const [otherTodos, resetTodos] = useSlice(resetTodosUpdate);

  const random = useSelector(
    (state: State) =>
      `(${state.legacy.toUpperCase()}) ${store.getState(fullNameSelect)}`
  );

  return <div>Conditional: {random}</div>;
}
